###Script to analyze Crab marina pooled data (23 populations) using poolfstat and BayPass R functions
...
setwd("/mnt/sdb/Snow_Crab_PoolSeq/Trimmed/")
#Load the Rdata
...
poolnames = c("MAR_B", "MAR_E","MAR_C","QUE","BBN","LILLY","CMA8A","CMA10B","CMA4","CMA5A","CMA2N","WCB","CMA3B","CMA6B","CMA3N200")
...
crabpools<-popsync2pooldata(sync.file = "SnowCrabPopool2_Q25.sync",
                                  poolsizes = poolsizes,
                                  poolnames = poolnames,
                                  min.rc = 3,
                                  min.cov.per.pool = 30,
...
crabpairFST.maf05 <- compute.pairwiseFST(crabpools, method = "Anova",
                                          ...
                                          verbose=TRUE)
write.csv(crabpairFST.maf05@PairwiseFSTmatrix,file="CrabPoolseq_PairwiseFST.csv", quote=F)

#Can subset pooldata if need be
#Crabpools.subset <- pooldata.subset(pooldata = Crabpools, min.maf = 0.05,verbose = TRUE) #this doesn't do anything since we filtered by a maf of 0.05 above
# #Results:
...
plot(crab.snp.fsts$sliding.windows.fst/1e4, 
     crab.snp.fsts$sliding.windows.fst$MultiLocusFst,
     xlab="Cumulative Position (Mb)",
     ylab="Multi-locus Fst")
#col=crab.snp.fsts$sliding.windows.fst$Chr,pch=16)
...
crab.snp.fsts.jacked<-computeFST(crabpools,method = "Anova",nsnp.per.bjack.block = 1000)

##### And we can compute pairwise FSTs among populations
crab.pair.fst <- compute.pairwiseFST(crabpools, method = "Anova",
                                    min.cov.per.pool = 20, 
                                    max.cov.per.pool = 500,
                                    min.maf = 0.01,
                                    output.snp.values = FALSE,
...
pairwisepools.fst <- crab.pair.fst@PairwiseFSTmatrix
write.csv(pairwisepools.fst,"Crab_PairwisePoolsFST.csv")
colnames(pairwisepools.fst) <- gsub("LAB","CMA_", colnames(pairwisepools.fst))
rownames(pairwisepools.fst) <- gsub("LAB","CMA_", rownames(pairwisepools.fst))
colnames(pairwisepools.fst) <- gsub("NS","MAR", colnames(pairwisepools.fst))
rownames(pairwisepools.fst) <- gsub("NS","MAR", rownames(pairwisepools.fst))
#Do a quick plot to see how it looks, which is....not great...
...
ggsave("~/Documents/GitHub/SnowCrabGenomics/figures/Poolseq_Fst_heatmap.png", plot = last_plot(), width = 10, height=8, 
       dpi=320)

#Now compute fstats

crab.fstats<-compute.fstats(x = crabpools,
                           nsnp.per.bjack.block = 1000,
                           computeDstat = T,
                           verbose = T)

#try to build an NJ tree
scaf.pops <-find.tree.popset(crab.fstats,verbose = T)
scaf.tree <- rooted.njtree.builder(fstats = crab.fstats,pop.sel = scaf.pops$pop.sets[1,],plot.nj = T,verbose = T)
...
pooldata2genobaypass(crabpools, writing.dir = getwd(), subsamplesize = -1)

#Let's also make a randomly subset BayPass file
pooldata2genobaypass(crabpools, writing.dir = getwd(), subsamplesize = 50000, subsamplingmethod = "random", prefix = "SCrabBayPassSUBSET")

#Let's also create a countdata object using the genobaypass file
#ZM.countdata <- genobaypass2countdata(genobaypass.file = "CrabAllPools.genobaypass",popnames = poolnames,min.maf = 0.05,verbose = T)
...
# Write to excel file
WriteXLS(uniq.LD, "CrabAllPools.LD.list.xls")
...
SG.omega=as.matrix(read.table("CrubPoolBayPass_mat_omega.out"))

# Identify pool names for plotting
colnames(SG.omega) <- c("MAR B", "MAR E", "MAR C", "QUE", "Bradelle Bank", "Lilly","CMA 8A", "CMA 10B", "CMA 4", "CMA 5A", "CMA 2N5440", "West Cape Breton", "CMA 3B", "CMA 6B","CMA 3N200")
rownames(SG.omega) <- c("MAR B", "MAR E", "MAR C", "QUE", "Bradelle Bank", "Lilly","CMA 8A", "CMA 10B", "CMA 4", "CMA 5A", "CMA 2N5440", "West Cape Breton", "CMA 3B", "CMA 6B","CMA 3N200")

pops= c("MAR B", "MAR E", "MAR C", "QUE", "Bradelle Bank", "Lilly","CMA 8A", "CMA 10B", "CMA 4", "CMA 5A", "CMA 2N5440", "West Cape Breton", "CMA 3B", "CMA 6B","CMA 3N200")
...
CrabBayPass.tree=as.phylo(hclust(as.dist(1-cor.mat**2)))
#use plot.phylo and its details to make a nice looking tree
plot(CrabBayPass.tree,
     type="phylo",
     edge.width=3,
     font=2,
     lab4ut="axial",
...
ZM.snp.res=read.table("CrabBayPass_summary_pi_xtx.out",h=T)

# Get the Pi Beta distribution for POD generation (pseudo-observed data set)
ZM.pi.beta.coef=read.table("CrabBayPass_summary_beta_params.out",h=T)$Mean
#0.247412 0.306792

# Upload original data to get read counts
ZM.data<-geno2YN("CrabAllPools.genobaypass")
