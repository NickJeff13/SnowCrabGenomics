...
pca <- read_table("data/exon.eigenvec", col_names = FALSE)
eigenval <- scan("data/exon.eigenval")
...
b <- b + scale_color_manual(values = as.vector(glasbey.colors(n=29)))+facet_wrap(~loc)
b <- b + coord_equal() + theme_light()
b + xlab(paste0("PC1 (", signif(pve$pve[1], 3), "%)")) + ylab(paste0("PC2 (", signif(pve$pve[2], 3), "%)"))

b + xlab(paste0("PC2 (", signif(pve$pve[2], 3), "%)")) + ylab(paste0("PC3 (", signif(pve$pve[3], 3), "%)"))

ggsave("Exon_PCA_facet_PC1PC2.tif",plot = b, device = "tif", path = "figures/", width = 16, height = 12, units = "in", dpi = 320)


### Now do the full SNP dataset - 9 million SNPs!!
pca <- read_table("data/snowcrab.eigenvec", col_names = FALSE)
eigenval <- scan("data/snowcrab.eigenval")
# sort out the pca data
# remove nuisance column
pca <- pca[,-1]
# set names
...
# plot pca
c <- ggplot(pca, aes(PC1, PC2, col = spp)) + geom_point(size = 3)
c <- c + scale_color_manual(values = as.vector(glasbey.colors(n=29)))+facet_wrap(~loc)
...
ggsave("AllSNPs_PCA_facet_PC1PC2.tif",plot = b, device = "tif", path = "figures/", width = 16, height = 12, units = "in", dpi = 320)

#non-faceted
e <- ggplot(pca, aes(PC1, PC2, col = spp)) + geom_point(size = 3)
e <- e + scale_color_manual(values = as.vector(glasbey.colors(n=29)))
e <- e + coord_equal() + theme_light()
